...
Data_DirPath_list = glob.glob('./CSV/Convolution/*')#'./CSV/Convolution/jyu',...
...
for Data_DirPath in Data_DirPath_list:
    ...
    TryNumPath_list = glob.glob(Data_DirPath+'/*')

    for TryNumPath in TryNumPath_list :#試行回数分、回転('1', '2', '3',...)

        TryNumSavePath = TryNumPath.replace('CSV/Convolution','Image')
        if os.path.isdir(TryNumSavePath) == False:#なければ保存用のディレクトリ作成
            os.mkdir(TryNumSavePath)

        CsvPath_list = glob.glob(TryNumPath+'/*.csv')#'./CSV/Convolution/jyu/1/CSP1427000000.csv',...
        ...
        for CsvPath in CsvPath_list:#フレーム分、回転('CSP1427000000.csv', 'CSP1427000001.csv',...)
            ...
            CTICla.csvtoimage(CsvPath,CsvSavePath)#生データのいらない部分を削除して整形
