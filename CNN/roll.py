...
for DirPath in DirPath_list:
    ...
    print TryNumPath_list
    for TryNumPath in TryNumPath_list :#試行回数分、回転('1', '2', '3',...)

        TryNumSavePath = TryNumPath.replace('Calibration','Roll_90')
        if os.path.isdir(TryNumSavePath) == False:#なければ保存用のディレクトリ作成
            os.mkdir(TryNumSavePath)
        TryNumSavePath = TryNumPath.replace('Calibration','Roll_180')
        if os.path.isdir(TryNumSavePath) == False:#なければ保存用のディレクトリ作成
            os.mkdir(TryNumSavePath)
        TryNumSavePath = TryNumPath.replace('Calibration','Roll_270')
        if os.path.isdir(TryNumSavePath) == False:#なければ保存用のディレクトリ作成
            os.mkdir(TryNumSavePath)

        CsvPath_list = glob.glob(TryNumPath+'/*.csv')#'./CSV/Calibration/1965/CSP1956000000.csv',...
        print CsvPath_list
