...
#./CSV/Cleaned/ 以下のデータを60*60から20*20に集約し ./CSV/Convolution/ に保存
#そのポイントの2秒間分(10データ)を[平均,中央,最頻,分散,標準偏差]の20*20の5つのcsvファイルを作成
import glob
import os, commands
import os.path
import csv
    ...
    def convolution(self,data_path,save_path):
        ...
        for line in range(20):
            for column in range(20) :
                ...
                convolution_map[line][column] = ave#3*3の領域の平均を代表値とする

        np.savetxt(save_path,convolution_map,delimiter=',')#集約後行列を保存

    #20*20*10から20*20に集約
    def calibrationMapGenerator(self,dirpath_list):
        mean_map = np.zeros([20, 20])#集約後の行列の初期化(平均)
        med_map = np.zeros([20, 20])#集約後の行列の初期化(中央)
        mode_map = np.zeros([20, 20])#集約後の行列の初期化(最頻値)
        var_map = np.zeros([20, 20])#集約後の行列の初期化(分散)
        sd_map = np.zeros([20, 20])#集約後の行列の初期化(標準偏差)
