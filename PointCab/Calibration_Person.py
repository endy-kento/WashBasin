    ...
    if args[2]=='new':
        ...
        for DirPath in tqdm(DirPath_list):
                ...
                for npyPath in npyPath_list:
                    ...
                    CabTes.cabTest(person,args[1],str(count),DirSavePath+'/')
                    count = count +1
        ...
        for npyPath in tqdm(npyPath_list):
            ...
            CabTes.Dentcab(Dentdata,args[1],str(count),npySavePath)
            count = count +1



    else:
        all_cab_deg1_map = np.load("./CabMaps_nonzero/all_cab_deg1_map.npy")#全てのサンプルデータを用いた回帰直線
        rep_cab_deg1_map = np.load("./CabMaps_nonzero/rep_cab_deg1_map.npy")#代表点を用いた回帰直線
